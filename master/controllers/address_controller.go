...
func IndexAdd(c *fiber.Ctx) error {
	id := c.Params("id")
	var data models.Loc
...
func UpdateAdd(c *fiber.Ctx) error {
	...
	var updateData models.Loc
	if err := c.BodyParser(&updateData); err != nil {
		return jsonResponse(c, fiber.StatusBadRequest, "Invalid input", err.Error())
	}

	if updateData.ID != 0 && updateData.ID != id {
		if err := models.DB.First(&models.Loc{}, updateData.ID).Error; err == nil {
...
func DeleteAdd(c *fiber.Ctx) error {
	...
	if models.DB.Delete(&models.Loc{}, id).RowsAffected == 0 {
